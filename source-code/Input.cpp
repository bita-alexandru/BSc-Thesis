...
std::unordered_map<std::string, std::string>* Input::GetStates()
{
	return &m_States;
}

std::unordered_map<std::string, bool>* Input::GetNeighbors()
{
	return &m_Neighbors;
}

std::unordered_map<std::string, std::any>* Input::GetRules()
{
	return &m_Rules;
}

std::deque<std::string>* Input::GetColors()
{
	return &m_Colors;
}

void Input::SetStates(std::vector<std::string> states)
{
    ...
    // update list display
    int nOfItems = m_ListStates->GetItemCount();
    int i = 0;
    for (i = 0; i < states.size(); i++)
    {
        ...
        if (i > nOfItems - 1)
        {
            m_ListStates->InsertItem(i, id);
            m_ListStates->SetItem(i, 1, state);
            m_ListStates->SetItemBackgroundColour(i, color);

            continue;
        }

        wxString itmId = m_ListStates->GetItemText(i, 0);
        wxString itmState = m_ListStates->GetItemText(i, 1);
        wxColour itmColor = m_ListStates->GetItemBackgroundColour(i);

        if (itmId != id)
        {
            m_ListStates->SetItem(i, 0, id);
        }
        if (itmState != state)
        {
            m_ListStates->SetItem(i, 1, state);
    ...
    while (i < nOfItems--)
    {
        m_ListStates->DeleteItem(i);
    }
}

void Input::SetNeighbors(std::vector<std::string> neighbors)
{
}
...
void Input::MakeColorAvailable(std::string& color)
{
...
void Input::MakeColorUnavailable(std::string& color)
{
    m_Colors.pop_front();
